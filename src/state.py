...
def initialize_master_recipes():
    ...
    id_to_title = {
        recipe["uid"]: recipe["title"] for recipe in original_master_recipes.values()
    }
    st.session_state["recipe_id_to_title"] = id_to_title

    st.session_state["cookbook"] = cheffrey.Cookbook(
        "master", recipe_dict=st.session_state["master_recipes"]
    )
...
def regen_recipe(i):
    ...
    if source == "random":
        ...
        new_recipe = st.session_state["searched_recipes"][
            st.session_state["searched_index"]
        ]
...
def unfavorite_recipe(recipe):
    favorites = st.session_state["user_config"][st.session_state["current_user"]][
        "favorites"
    ]
    new_favorites = [r for r in favorites if recipe["title"] != r]
    st.session_state["user_config"][st.session_state["current_user"]][
        "favorites"
    ] = new_favorites
...
def favorite_recipe(recipe):
    ...
    if (
        recipe["title"]
        not in st.session_state["user_config"][st.session_state["current_user"]][
            "favorites"
        ]
    ):
        st.session_state["user_config"][st.session_state["current_user"]][
